...
public class Lab5 extends GLJPanel implements GLEventListener, KeyListener{
	...
	public void Corkscrew(GL2 gl2,int radius){
        gl2.glClear(GL2.GL_COLOR_BUFFER_BIT);

        int numVertices = 5000;
        double angleIncrement = 6.5 * Math.PI / numVertices; //10 obrotów
    ...
    public void Pyramid(GL2 gl2,double size){
    	//sekcja œcian
        ...
        for (int i=0; i<n;i++)
        {
            gl2.glColor3f(7.0f, 0.0f, 2.0f);
            gl2.glVertex3f((float) (Math.cos(i*2*Math.PI/n)*size), -3, (float) (Math.sin(i*2*Math.PI/n)*size));
            gl2.glColor3f(2.0f, 0.0f, 7.0f);
            gl2.glVertex3f((float) (Math.cos((i+1)*2*Math.PI/n)*size), -3, (float) (Math.sin((i+1)*2*Math.PI/n)*size));
        }
        gl2.glEnd();
    }
